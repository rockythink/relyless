...
- `bun test tests/content-reader-lifecycle.test.js`：1 通过、28 断言；原页结果在视图切换、局部清理和 `SS_REFRESH` 后保留同一译文节点，新 token 可继续，显式清除移除结果。并覆盖视图容器被宿主移除后原页 inert/滚动锁恢复；既有读者/后台专项检查同样覆盖费用确认、译文清理和预算估算。
