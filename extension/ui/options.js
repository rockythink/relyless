...
function optionsRenderDetection(){const value=optionsDetectionSettings();const radio=document.querySelector(`input[name="domain-detection-mode"][value="${value.mode}"]`);if(radio)radio.checked=true;optionsEls.detectionChatgpt.hidden=value.mode!=='chatgpt';optionsEls.detectionApi.hidden=value.mode!=='api';optionsEls.detectionJev.hidden=value.mode!=='jev';optionsEls.detectionUseTranslationApi.checked=value.useTranslationApi!==false;optionsEls.detectionApiFields.hidden=optionsEls.detectionUseTranslationApi.checked;optionsRenderModels(optionsEls.detectionSubscriptionModel,value.subscriptionModel||'',optionsEls.detectionModelNote);if(!optionsDetectionDirty){optionsEls.detectionApiModel.value=value.apiModel||'';optionsEls.detectionApiUrl.value=value.api?.baseUrl||'https://api.openai.com/v1';optionsEls.detectionApiKey.value='';}const hasKey=Boolean(value.api?.apiKey);optionsEls.detectionApiKey.placeholder=hasKey?'已保存；同源留空保持':'输入单独的 API Key';optionsEls.detectionKeyState.textContent=hasKey?'单独的识别密钥已保存在本机。':'留空不会复制辅助 API 密钥。';optionsEls.clearDetectionKey.disabled=!hasKey;
  const jevProvider=JUDGMENT_PROVIDER_IDS.includes(value.jevProvider)?value.jevProvider:'requesty',jevDefaults=judgmentProviderDefaults(jevProvider);
  optionsEls.detectionJevProvider.value=jevProvider;optionsEls.detectionJevProviderNote.hidden=jevProvider!=='siliconflow-systemone';
  optionsEls.detectionJevModel.placeholder=jevDefaults.model;optionsEls.detectionJevUrl.placeholder=jevDefaults.baseUrl;
  if(!optionsDetectionDirty){optionsEls.detectionJevModel.value=value.jevModel||jevDefaults.model;optionsEls.detectionJevUrl.value=value.jevBaseUrl||jevDefaults.baseUrl;optionsEls.detectionJevKey.value='';}
  const hasJevKey=Boolean(value.jevApiKey);
  optionsEls.detectionJevKey.placeholder=hasJevKey?'已保存；留空保持':'输入判定接入的 API Key';
  optionsEls.detectionJevKeyState.textContent=hasJevKey?'Jev 密钥已保存在本机。':'支持填入所选接入平台的 API Key。';
...
optionsEls.pdfReader?.addEventListener('change',()=>void optionsSavePatch({pdfReader:optionsEls.pdfReader.checked},optionsEls.pdfReader.checked?'已开启 PDF 阅读器':'已关闭 PDF 阅读器'));
...
document.querySelectorAll('input[name="domain-detection-mode"]').forEach(input=>input.addEventListener('change',()=>{optionsDetectionDirty=true;optionsEls.detectionChatgpt.hidden=input.value!=='chatgpt';optionsEls.detectionApi.hidden=input.value!=='api';optionsEls.detectionJev.hidden=input.value!=='jev';}));
