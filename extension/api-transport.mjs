...
async function probeOutputMode(service,protocol,signal,onUsage,thinking){
  aborted(signal);
  ...
  const body=protocol==='chat'
    ?{model:service.model,max_tokens:128,...chatThinking(service,thinking),response_format:{type:'json_schema',json_schema:{name:format.name,strict:true,schema}},messages:[{role:'user',content:prompt}]}
    :{model:service.model,max_output_tokens:128,...responsesThinking(service,thinking),text:{format},input:prompt};
  let response;
...
async function outputMode(service,protocol,signal,onUsage,thinking){
  aborted(signal);
  ...
  const identity=JSON.stringify([service.providerId,endpointFor(service,protocol).href,service.model,service.apiKey||'',protocol==='chat'?chatThinking(service,thinking):responsesThinking(service,thinking)]);
  ...
  if(!entry||entry.controller.signal.aborted){
    ...
    entry.promise=probeOutputMode(service,protocol,entry.controller.signal,onUsage,thinking).then(mode=>{
...
// 思考模式档位：'auto' 沿用既有启发式；'off' 对无既有约定的服务补发通用关闭参数；
// low/medium/high 跳过恒思考拒绝并按服务商已知参数名开启推理。budget_tokens 必须低于请求 max_tokens。
const THINKING_LEVELS=new Set(['low','medium','high']);
const THINKING_BUDGETS=Object.freeze({low:1024,medium:4096,high:7168});
function thinkingLevel(service){const value=service?.options?.thinking;return value==='off'?'off':THINKING_LEVELS.has(value)?value:'auto';}
function chatThinkingAuto(service){const id=service.providerId,model=service.model||'',base=service.baseUrl||'';
...
function chatThinking(service,level='auto'){const auto=chatThinkingAuto(service);
  if(level==='auto')return auto;
  if(level==='off')return Object.keys(auto).length?auto:{reasoning_effort:'none'};
  const id=service.providerId;
  if(id==='alibaba'||id==='siliconflow')return {enable_thinking:true};
  if(['deepseek','moonshotai','volcengine','minimax'].includes(id))return {thinking:{type:'enabled'}};
  if(id==='openai-compatible'&&(/deepseek/i.test(service.model||'')||/deepseek/i.test(service.baseUrl||'')))return {thinking:{type:'enabled'}};
  if(id==='openrouter')return {reasoning:{effort:level}};
  if(id==='togetherai')return {reasoning:{enabled:true}};
  return {reasoning_effort:level};
}
function responsesThinkingAuto(service){const model=service.model||'';if(service.providerId==='open-responses')return {reasoning:{effort:'none'}};if(service.providerId==='xai'&&/^grok-4\.3(?:$|-)/i.test(model))return {reasoning:{effort:'none'}};return (service.providerId==='openai'||service.providerId==='azure')&&/^gpt-(?:[5-9]|[1-9]\d)/i.test(model)?{reasoning:{effort:'none'}}:{};}
function responsesThinking(service,level='auto'){const auto=responsesThinkingAuto(service);
  if(level==='auto')return auto;
  if(level==='off')return Object.keys(auto).length?auto:{reasoning:{effort:'none'}};
  return {reasoning:{effort:level}};
}
// Gemini thinkingBudget：0 关闭，-1 为官方动态模式（档位粒度交由模型自行分配）。
function googleThinking(service,level='auto'){
  if(level==='auto')return /gemini-2\.5-(?:flash|flash-lite)(?:$|[-_:])/i.test(service.model||'')?{thinkingConfig:{thinkingBudget:0}}:{};
  if(level==='off')return {thinkingConfig:{thinkingBudget:0}};
  return {thinkingConfig:{thinkingBudget:-1}};
}
function anthropicThinking(service,level){
  if(level==='auto')return /^claude-(?:haiku|sonnet|opus)-4/i.test(service.model||'')?{thinking:{type:'disabled'}}:{};
  if(level==='off')return {thinking:{type:'disabled'}};
  return {thinking:{type:'enabled',budget_tokens:THINKING_BUDGETS[level]}};
}
function bedrockThinking(service,level){
  if(level==='auto')return /anthropic\.claude-(?:haiku|sonnet|opus)-4/i.test(service.model||'')?{additionalModelRequestFields:{thinking:{type:'disabled'}}}:{};
  if(level==='off')return {additionalModelRequestFields:{thinking:{type:'disabled'}}};
  return {additionalModelRequestFields:{thinking:{type:'enabled',budget_tokens:THINKING_BUDGETS[level]}}};
}
function cohereThinking(service,level){
  if(level==='auto')return /reasoning/i.test(service.model||'')?{thinking:{type:'disabled'}}:{};
  if(level==='off')return {thinking:{type:'disabled'}};
